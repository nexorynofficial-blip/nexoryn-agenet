 ...
 *   2. Calls `onAutofill` with the result, so the PARENT page
 *      (ProjectForm.tsx) can apply it to its own real form state —
 *      the admin then sees the actual fields filled in and reviews
 *      them in place.
 *
 * Images are NOT handled here — the admin adds those manually via
 * the dashboard's existing photo picker, same as always.
 *
...
export interface AutofillPayload {
  ...
  description: string;
  tags: string[];
  /** Shaped like caseStudy in PORTFOLIO_FORM_REFERENCE.md §4.2, minus gallery/photoId — images are handled manually, not by this widget. */
...
interface FloatingAgentWidgetProps {
  ...
  /** Called with everything assembled once extraction finishes.
...
const STYLE_SHEET = `
@import url('https://fonts.googleapis.com/css2?family=Manrope:wght@400;500;600;700&family=Montserrat:wght@800&family=Chakra+Petch:wght@400;500;600;700&display=swap');
.nxai-root {
  --night: #150c05;
  --amber-deep: #3a1c05;
  --glow: #c85a12;
      ...
      {open && (
        <div className="nxai-panel">
          ...
          <div className="nxai-header">
            <span className="nxai-title">AI Project Assistant</span>
